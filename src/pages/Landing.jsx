  ...
  return (
    <Layout>
      ...
      <section className="min-h-[92vh] flex flex-col justify-center bg-white border-b border-neutral-200">
        <div className="max-w-screen-xl mx-auto px-8 w-full py-20">
          <div className="max-w-5xl">
            <p className="animate-hero-1 text-xs font-semibold uppercase tracking-[0.2em] text-neutral-400 mb-8">
              {t('landing_badge')}
            </p>
            <h1 className="animate-hero-2 text-6xl sm:text-7xl lg:text-[5.5rem] font-black text-neutral-950 leading-[1.0] tracking-tight mb-8">
              {t('landing_headline').split('\n').map((line, i) => (
                <span key={i} className="block">{line}</span>
              ))}
            </h1>
            <p className="animate-hero-3 text-neutral-500 text-lg sm:text-xl max-w-2xl leading-relaxed mb-12">
              {t('landing_sub')}
            </p>
            <div className="animate-hero-4 flex flex-col sm:flex-row items-start sm:items-center gap-4">
              <Link
                to="/intake"
                className="group inline-flex items-center gap-2 bg-neutral-950 hover:bg-neutral-800 text-white font-semibold text-base px-8 py-4 rounded-md transition-all hover:-translate-y-0.5 hover:shadow-lg"
              >
                {t('landing_cta')}
            ...
            <p className="animate-hero-4 text-neutral-400 text-sm mt-6">{t('landing_cta_sub')}</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white border-b border-neutral-200">
        <div className="max-w-screen-xl mx-auto px-8 py-28">
          <div className="reveal mb-16">
            ...
            <h2 className="text-4xl sm:text-5xl font-black text-neutral-950">{t('landing_how_title')}</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-neutral-200 stagger">
            {STEPS.map(({ num, titleKey, descKey, icon }) => (
              <div key={num} className="reveal bg-white p-10 hover:bg-neutral-50 transition-colors group">
                <div className="flex items-start justify-between mb-8">
                  <p className="text-6xl font-black text-neutral-100 select-none group-hover:text-neutral-200 transition-colors">{num}</p>
                  <div className="text-neutral-300 group-hover:text-neutral-500 transition-colors">{icon}</div>
                </div>
      ...
      <section className="bg-neutral-50 border-b border-neutral-200">
        <div className="max-w-screen-xl mx-auto px-8 py-28">
          <div className="reveal mb-16">
            ...
            <h2 className="text-4xl sm:text-5xl font-black text-neutral-950">{t('landing_programs_title')}</h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 stagger">
            {PROGRAMS.map(p => (
              <div
                key={p.id}
                className="reveal bg-white border border-neutral-200 rounded-md p-6 flex flex-col gap-3 hover:border-neutral-400 hover:-translate-y-0.5 hover:shadow-sm transition-all"
              >
                <div className="w-8 h-8 rounded bg-neutral-950 flex items-center justify-center">
                  <span className="text-white text-xs font-black">{(p.name || p.id).slice(0, 2).toUpperCase()}</span>
                </div>
      ...
      <section className="bg-white border-b border-neutral-200">
        <div className="max-w-screen-xl mx-auto px-8 py-28">
          <div className="reveal mb-16">
            ...
            <h2 className="text-4xl sm:text-5xl font-black text-neutral-950">What makes us different</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-neutral-200 stagger">
            {DIFFERENTIATORS.map(({ title, desc, icon }) => (
              <div key={title} className="reveal bg-white p-8 hover:bg-neutral-50 transition-colors group">
                <div className="text-neutral-300 group-hover:text-neutral-950 transition-colors mb-5">{icon}</div>
      ...
      <section className="bg-neutral-950">
        <div className="max-w-screen-xl mx-auto px-8 py-32 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-12">
          ...
          <div className="reveal flex-shrink-0">
            <Link
              to="/intake"
              className="group inline-flex items-center gap-2 bg-white hover:bg-neutral-100 text-neutral-950 font-bold text-base px-10 py-4 rounded-md transition-all hover:-translate-y-0.5 hover:shadow-lg"
            >
              {t('landing_cta')}
              <svg className="w-4 h-4 transition-transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
