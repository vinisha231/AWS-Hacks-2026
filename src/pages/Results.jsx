...
function ProgramCard({ program, lang, onApply }) {
  ...
  return (
    <div className="reveal bg-white border border-neutral-200 rounded-lg overflow-hidden hover:-translate-y-0.5 hover:shadow-md hover:border-neutral-400 transition-all duration-200">
      <div className="p-6">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex items-center gap-3">
            <span className="w-10 h-10 rounded-md bg-neutral-950 flex items-center justify-center text-xs font-black text-white flex-shrink-0">
              {program.category[0].toUpperCase()}
            </span>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="font-black text-lg text-neutral-950">{t(program.nameKey)}</span>
                {program.waitlist && (
          ...
          <div className="text-right flex-shrink-0">
            ...
            <div className="text-xs text-neutral-400">{t('results_annual')}</div>
          </div>
        </div>

        <p className="text-neutral-600 text-sm leading-relaxed mb-4">{t(program.descKey)}</p>

        {/* Why you qualify */}
        <div className="text-xs rounded-md px-4 py-3 mb-4 flex items-start gap-2 bg-neutral-50 border border-neutral-200 text-neutral-700">
        ...
        <button
          ...
          className="text-sm text-neutral-500 hover:text-neutral-700 font-medium flex items-center gap-1 mb-3 transition-colors"
        >
        ...
        {expanded && (
          <ul className="mb-4 space-y-1.5 animate-fade-in">
            {program.documents.map(doc => (
              <li key={doc} className="flex items-start gap-2 text-sm text-neutral-600">
        ...
        <button
          ...
          className="w-full py-3 rounded-md font-bold text-sm text-white bg-neutral-950 hover:bg-neutral-800 transition-all hover:-translate-y-0.5 hover:shadow-md"
        >
          {t('results_prefill')} →
        </button>
      </div>
    ...
    <Layout>
      ...
      <div className="border-b border-neutral-200 bg-white py-16 px-6">
        <div className="max-w-screen-xl mx-auto px-6 lg:px-12 text-center">
          ...
          {results.length > 0 && (
            <div className="inline-flex items-center gap-3 bg-neutral-50 border border-neutral-200 rounded-lg px-6 py-4 mt-2">
              <span className="text-4xl font-black text-neutral-950">{fmt(total)}</span>
