...
function ProgramCard({ program, lang, onApply, answers }) {
  ...
  const eligClass = confidence >= 4
    ? 'bg-emerald-50 border-emerald-200 text-emerald-800'
    : confidence >= 3
      ? 'bg-amber-50 border-amber-200 text-amber-800'
      : 'bg-red-50 border-red-200 text-red-800'
  const eligIconColor = confidence >= 4 ? '#16a34a' : confidence >= 3 ? '#d97706' : '#dc2626'

  return (
    <div className="reveal bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
      <div className="p-6">
        {/* Header */}
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex items-center gap-3">
            <span className="w-10 h-10 rounded-md bg-emerald-100 border border-emerald-200 flex items-center justify-center text-xs font-black text-emerald-700 flex-shrink-0">
              {program.category[0].toUpperCase()}
            </span>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="font-black text-lg text-gray-900">{t(program.nameKey)}</span>
                {program.waitlist && (
          ...
          <div className="text-right flex-shrink-0">
            ...
            <div className="text-xs text-gray-400">{t('results_annual')} · estimated range</div>
          </div>
        </div>

        <p className="text-gray-600 text-sm leading-relaxed mb-4">{t(program.descKey)}</p>

        {/* Time / difficulty badges */}
        <div className="flex items-center gap-2 mb-4">
          {program.timeToBenefit && (
            <span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full">⏱ {program.timeToBenefit}</span>
          )}
          {program.difficulty && (
            <span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full">⚙️ {program.difficulty}</span>
          )}
        </div>

        {/* Eligibility confidence banner */}
        <div className={`text-xs rounded-md px-4 py-3 mb-4 flex flex-col gap-2 border ${eligClass}`}>
          <div className="flex items-start gap-2">
            <svg className="w-4 h-4 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20" style={{ color: eligIconColor }}>
              <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
            </svg>
            <span className="font-semibold">{eligLabel} — {t(program.whyKey)}</span>
          </div>
          <div className="opacity-80 text-xs">
            Confidence: <span className="font-semibold">{confidenceLabel(confidence)}</span>. What could disqualify you: <span className="font-semibold">{(program.cons && program.cons.length > 0) ? program.cons.join('; ') : 'Income too high, non-citizen status, missing documentation'}</span>
          </div>
        </div>
        ...
        <button
          ...
          className="text-sm text-gray-500 hover:text-gray-900 font-medium flex items-center gap-1 mb-3 transition-colors"
        >
        ...
        {expanded && (
          <div className="mb-4 animate-fade-in space-y-4">
            ...
            {program.documents?.length > 0 && (
              <div>
                <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1.5">{t('results_documents')} needed ({program.documents.length})</p>
                {documentContext(program, answers) && (
                  <p className="text-xs text-gray-500 mb-3">{documentContext(program, answers)}</p>
                )}
...
export default function Results() {
    ...
    return (
      <Layout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
          <p className="text-gray-500 mb-6 text-lg">{t('results_no_data')}</p>
          <Link to="/intake" className="bg-gray-900 text-white font-bold px-6 py-3 rounded-md hover:bg-gray-700 transition-colors">
      ...
      <Layout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 text-center px-4">
          <div className="relative">
            <div className="w-20 h-20 rounded-full border-4 border-gray-200 border-t-emerald-600 animate-spin" />
          </div>
          <p className="text-gray-700 font-semibold text-xl">{t('results_loading')}</p>
          <div className="flex flex-col gap-2 text-sm text-gray-400 max-w-xs">
            {[t('results_loading_snap'), t('results_loading_medicaid'), t('results_loading_housing'), t('results_loading_values')].map((msg, i) => (
              <div key={msg} className="flex items-center gap-2 animate-fade-in" style={{ animationDelay: `${i * 0.2}s` }}>
                <span className="w-1.5 h-1.5 rounded-full bg-gray-300 inline-block flex-shrink-0 mt-1.5" />
                {msg}
              </div>
            ))}
          </div>
  ...
  return (
    <Layout>
      {/* Results header */}
      <div className="border-b border-gray-200 bg-gray-50 py-16 px-6">
        <div className="max-w-screen-xl mx-auto px-6 lg:px-12 text-center">
          {name && <p className="text-gray-500 text-sm font-medium mb-2">{t('results_for_name', { name })}</p>}
          ...
          {results.length > 0 && (
            <div className="inline-flex items-center gap-3 bg-white border border-gray-200 rounded-xl shadow-sm px-6 py-4 mt-2">
              <span className="text-4xl font-black text-emerald-700">{fmt(total)}</span>
              <span className="text-gray-500 text-left">
                <div className="font-bold text-gray-700">{t('results_per_year')}</div>
                <div className="text-sm">{t('results_benefits_qualify')}</div>
              </span>
            </div>
          )}
        </div>
      ...
      {/* Summary bar */}
      {results.length > 0 && (
        <div className="bg-white border-b border-gray-200 py-4 px-4 sticky top-16 z-20 shadow-sm">
          <div className="max-w-screen-xl mx-auto px-6 lg:px-12 flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div className="flex gap-3">
              <Link
                to="/intake"
                className="text-gray-500 hover:text-gray-900 text-sm font-medium px-4 py-2 rounded-md border border-gray-300 hover:border-gray-500 transition-colors"
              >
                {t('results_restart')}
              </Link>
              <button
                onClick={handleSave}
                className="text-sm font-bold px-4 py-2 rounded-md transition-all bg-emerald-600 hover:bg-emerald-500 text-white"
              >
      ...
      <div className="max-w-screen-xl mx-auto px-6 lg:px-12 py-10">
        {results.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg mb-6">{t('results_none')}</p>
            <Link to="/intake" className="bg-gray-900 text-white font-bold px-6 py-3 rounded-md hover:bg-gray-700 transition-colors">
              {t('results_restart')}
            </Link>
          </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            {/* Bottom save CTA */}
            <div className="reveal bg-gray-900 rounded-xl p-8 text-center">
              <h3 className="font-black text-white text-lg mb-2">{t('results_dont_lose')}</h3>
              <p className="text-gray-400 text-sm mb-4">{t('results_save_desc')}</p>
              <button
                onClick={handleSave}
                className="bg-emerald-500 hover:bg-emerald-400 text-white font-bold px-6 py-3 rounded-md transition-colors"
              >
