...
export function AdvocatePanel({ program, onClose }) {
  ...
  // Auto-play caseworker messages via Polly (only if user already interacted with audio)
  useEffect(() => {
    ...
    if (last.role === 'assistant') {
      ...
      speakText(last.content, lang)
        .catch(err => {
          console.warn('Polly autoplay blocked (browser policy). Use the 🔊 button to play:', err)
          setSpeaking(false)
        })
              ...
              {program.documents?.length > 0 ? (
                <ul className="space-y-3">
                  {program.documents.map((doc, i) => (
                    <li key={i} className="flex items-start gap-3 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                      <span className="w-6 h-6 rounded-full border-2 border-gray-300 flex-shrink-0 mt-0.5" />
                ...
                <ul className="space-y-3">
                  {['Government-issued photo ID (drivers license or passport)', 'Proof of income (recent pay stubs or tax return)', 'Proof of residency (utility bill or lease agreement)', 'Social Security numbers for all household members', 'Bank statements from the last 30-60 days'].map((doc, i) => (
                    <li key={i} className="flex items-start gap-3 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                      <span className="w-6 h-6 rounded-full border-2 border-gray-300 flex-shrink-0 mt-0.5" />
                      <span className="text-sm text-gray-800 font-medium">{doc}</span>
                    </li>
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div className="space-y-3">
                {(program.cons?.length > 0 ? program.cons : [
                  'Income too high — verify the exact threshold for your household size',
                  'Non-citizen or immigration status — some programs require specific visa categories',
                  'Missing or expired documentation — ensure all documents are current',
                  'Already receiving a conflicting benefit — check for program exclusions',
                  'Missed the application deadline or renewal window',
                ]).map((reason, i) => (
                  <div key={i} className="flex items-start gap-3 p-4 bg-red-50 border border-red-100 rounded-lg">
                    <span className="text-red-500 font-black flex-shrink-0 text-lg leading-none mt-0.5">×</span>
                    <div>
                      <p className="text-sm text-gray-900 font-semibold">{reason}</p>
                    </div>
                  </div>
                ))}
              </div>
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 space-y-2">
                ...
                <ul className="text-sm text-amber-700 space-y-1">
                  ...
                  <li>• Gather additional documentation before the appeal hearing</li>
                  <li>• Contact a local legal aid society for free appeal assistance</li>
                </ul>
              </div>
            </div>
            <div className="flex-shrink-0 px-6 pb-6 pt-3 border-t border-gray-200">
                ...
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} items-end gap-1`}>
                  ...
                  {msg.role === 'assistant' && (
                    <button
                      ...
                      title="Play audio"
                      className="flex-shrink-0 mb-0.5 flex items-center gap-1 px-2 py-1 rounded-md bg-emerald-50 border border-emerald-200 text-emerald-700 hover:bg-emerald-100 transition-colors text-xs font-semibold"
                    >
                      🔊
                    </button>
                  )}
