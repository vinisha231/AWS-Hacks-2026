    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-52 bg-white border border-slate-200 rounded-lg shadow-lg z-20">
...
export default function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <Link to="/" className="flex-shrink-0">
            <span className="font-semibold text-xl text-slate-900 tracking-tight">Compass</span>
          </Link>
      ...
      <footer className="border-t border-slate-200 bg-slate-50 py-4 px-4 text-center text-xs text-slate-400">
        {t('landing_footer')}
      </footer>
    </div>
  )
...
function NavLink({ to, label, badge, active }) {
  return (
    ...
    >
      {label}
      {badge > 0 && (
        <span className="bg-blue-700 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">
          {badge}
        </span>
      )}
    </Link>
